...
from api.models import (
    ...
    ComparableProperty,
    PropertyWithScores,
    PropertyWithScoresListResponse,
)
from api.database import (
    ...
    get_properties_with_scores,
    get_properties_with_scores_count,
)
...
def _convert_to_property_detail(property_data: dict) -> PropertyDetail:
    ...
    if property_data.get("total_months") is not None:
        reviews = PropertyReviews(
            ...
            review_pct_stayed_with_kids=property_data.get(
                "review_pct_stayed_with_kids"
            ),
            review_pct_group_trip=property_data.get("review_pct_group_trip"),
            review_pct_stayed_with_a_pet=property_data.get(
...
async def list_properties(
    ...
    min_occupancy: Optional[float] = Query(
        None, ge=0, le=100, description="Minimum occupancy (0-100)"
    ),
    max_occupancy: Optional[float] = Query(
        None, ge=0, le=100, description="Maximum occupancy (0-100)"
    ),
    min_rating: Optional[float] = Query(
        None, ge=0, le=5, description="Minimum rating (0-5)"
    ),
    is_guest_favorite: Optional[bool] = Query(
        None, description="Filter by guest favorite status"
    ),
    is_reliable_data: Optional[bool] = Query(
        None, description="Filter by reliable data status"
    ),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error listing properties: {str(e)}"
        )


@router.get(
    "/properties/with-scores",
    response_model=PropertyWithScoresListResponse,
    tags=["properties"],
)
async def list_properties_with_scores(
    market_id: Optional[str] = Query(None, description="Filter by market ID"),
    bedrooms: Optional[float] = Query(None, description="Filter by bedroom count"),
    min_revenue: Optional[float] = Query(None, ge=0, description="Minimum revenue"),
    ...
    limit: int = Query(
        100, ge=1, le=500, description="Maximum number of properties to return (1-500)"
    ),
    sort_by: Optional[str] = Query(None, description="Field to sort by"),
    sort_order: str = Query("desc", pattern="^(asc|desc)$", description="Sort order"),
):
    """
    List properties with investment scores from materialized view.

    This endpoint queries the mv_properties_with_scores materialized view for
    fast access to properties with their investment scores and performance metrics.
    ...
    Args:
        ...
        limit: Maximum number of properties to return (1-500)
        sort_by: Field to sort by
        sort_order: Sort order ('asc' or 'desc')

    Returns:
        List of properties with investment scores
    """
    try:
        logger.info(
            f"Fetching properties with scores: market_id={market_id}, bedrooms={bedrooms}, min_revenue={min_revenue}, min_total_score={min_total_score}, opportunity_tier={opportunity_tier}, is_top_opportunity={is_top_opportunity}, limit={limit}"
        )

        # Get properties with scores
        properties_data = get_properties_with_scores(
            market_id=market_id,
            bedrooms=bedrooms,
            min_revenue=min_revenue,
        ...
        logger.info(
            f"Retrieved {len(properties_data)} properties with scores from database"
        )

        # Convert to Pydantic models
        properties = [PropertyWithScores(**prop) for prop in properties_data]

        logger.info(
            f"Successfully converted {len(properties)} properties to Pydantic models"
        )

        return PropertyWithScoresListResponse(
            success=True,
        ...
        logger.error(f"Error listing properties with scores: {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Error listing properties with scores: {str(e)}"
        )
...
async def get_property(property_id: str):
    ...
    try:
        ...
        return PropertyDetailResponse(success=True, data=property_detail)
...
@router.get(
    "/properties/{property_id}/analysis",
...
async def get_property_analysis_endpoint(
    ...
    include_comparables: bool = Query(
        True, description="Include comparable properties nearby"
    ),
):
    """
    ...
    try:
        ...
        return PropertyAnalysisResponse(
            success=True, data=property_analysis, comparables=comparables
        )
    except HTTPException:
        raise
    except Exception as e:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting property analysis: {str(e)}"
        )
