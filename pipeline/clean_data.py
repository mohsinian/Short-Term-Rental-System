...
def clean_market_data(df, market_name):
    ...
    df_clean['listing_name'] = df_clean['Listing Name'].fillna(df_clean['TITLE'])\
    ...
    if 'LATITUDE' in df_clean.columns:
        df_clean['latitude'] = df_clean['LATITUDE'].fillna(df_clean.get('latitude', np.nan))
    ...
    if 'LONGITUDE' in df_clean.columns:
        df_clean['longitude'] = df_clean['LONGITUDE'].fillna(df_clean.get('longitude', np.nan))
    ...
    df_clean['total_reviews_clean'] = df_clean['total_reviews']\
                                        .fillna(df_clean['review_total_reviews'])\
                                        .fillna(df_clean['reviewsCount'])
    
    # Beds: Merge
    if 'number_of_beds' in df_clean.columns:
        df_clean['beds'] = df_clean['number_of_beds'].fillna(df_clean.get('beds', np.nan))
    ...
    if 'BATHROOMS' in df_clean.columns:
        df_clean['bathrooms'] = df_clean['BATHROOMS'].fillna(df_clean.get('baths', pd.Series([np.nan]*len(df_clean))).apply(extract_numeric_from_string))
    ...
    if 'Data Quality Category' in df_clean.columns:
        df_clean['is_reliable_data'] = df_clean['Data Quality Category'].isin(['Good Data', 'Possibly Good Data'])
